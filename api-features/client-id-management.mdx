...
description: "Create and manage Client IDs for frontend authentication, domain whitelisting, and payment destination configuration"
---

## Overview

Client IDs identify applications using the Request Network API. Domain-restricted Client IDs support frontend/browser integrations; Client IDs without allowed domains support server-side applications.

**Key differences from API keys:**

| | API Key | Client ID |
|---|---------|-----------|
| **Use case** | Backend integrations | Frontend/browser or server-side applications |
| **Auth header** | `x-api-key` | `x-client-id` (plus `Origin` when domain-restricted) |
...
When a Client ID is bound to a [payee destination](/api-features/payee-destinations), a standard `POST /v2/request` call made with that Client ID can omit the payee. Request Network resolves it from the bound destination. This lets an application create requests without including wallet details each time.

```
Application (with its Client ID) → POST /v2/request (no payee field)
  → API resolves payee from Client ID's bound destination
  → Payment goes to the configured wallet
```

<Note>
This is separate from a formal [orchestrator](/orchestrators/overview). For a Client ID linked to an orchestrator, creating a Secure Payment link or payout requires both `x-client-id` and `x-orchestrator-key`. A request using that linked Client ID alone is rejected. See [paired authentication](/orchestrators/client-id-linking#create-payment-links-for-a-platform).
</Note>
...
| **Auth API** (`POST /v1/client-ids`) | Automation, dynamic provisioning, multi-tenant applications |
