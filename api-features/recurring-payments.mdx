...
Recurring payments rely on ERC-20 permit (`EIP-2612`) signatures and are therefore **EVM-only**.
...
<Note>
Tron does not support ERC-20 permit semantics, so recurring payments are not available on Tron. For Tron, send single payouts on a schedule from your own backend.
</Note>
