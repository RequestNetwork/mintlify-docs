...
A signature-verifying Express handler with delivery-ID deduplication. It verifies against the **raw** body, uses constant-time comparison, and lets Request Network retry a failed handler.
...
app.post(
  ...
  async (req, res) => {
    ...
    // 2. Idempotency: dedupe deliveries that completed successfully.
    const completedKey = `webhook:${deliveryId}`;
    if (!isTest && (await redis.exists(completedKey))) {
    ...
    try {
      ...
      // Record completion only after downstream processing succeeds.
      await redis.set(completedKey, "1", "EX", 86400); // 24h
...
async function handleEvent(event: any) {
  switch (event.event) {
    ...
    case "secure_payment.user_event":
      await recordFunnelStep(event.securePaymentToken, event.userEvent);
      break;
...
Webhook delivery is at least once, not exactly once. The delivery ID prevents ordinary duplicate deliveries, but downstream work must also be idempotent: a process can fail after a side effect but before it returns `200`.
