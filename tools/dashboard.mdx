...
When the Tron feature flag is enabled, the login screen offers EVM and Tron as parallel tabs. Without the flag, only EVM sign-in is shown.

<Tabs>
<Tab title="EVM">
Connect MetaMask, Coinbase Wallet, or WalletConnect. The Dashboard issues a SIWE challenge; you sign with `personal_sign`.
...
The session is a wallet-bound cookie with a 15-minute idle timeout, shared across `dashboard.request.network`, `auth.request.network`, and `api.request.network`.
