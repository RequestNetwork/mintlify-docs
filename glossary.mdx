...
In the Reference-based Payment Networks, Payments are linked to Requests via a `paymentReference` which is derived from the `requestId` and payment recipient address. For details see [Payment Detection](/api-features/payment-detection).
...
A "swap-to-pay" payment is where the payment sender sends one currency but the payment recipient receives a different currency. For details see [Crosschain Payments](/api-features/crosschain-payments).

### Swap-to-Conversion Payment

A "swap-to-conversion" payment is where the request is denominated in currency A, the payer sends currency B and the payee receives currency C. For details see [Crosschain Payments](/api-features/crosschain-payments).
