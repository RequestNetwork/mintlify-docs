...
<AccordionGroup>
  ...
  <Accordion title="Are requests in Request Network stored fully on-chain?">
  ...
  <Accordion title="Can a Request Node modify a request's contents before persisting it to IPFS and on-chain?">
  ...
  <Accordion title="Does Request Network support crosschain payments where the payer sends funds on one chain and the payee receives funds on a different chain?">
    No. Request Network does not currently support crosschain payments. All of the supported payment types involve the payer sending funds and the payee receiving funds on the same chain.
  </Accordion>
