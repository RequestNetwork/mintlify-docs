...
description: "Quick setup guide to get your Client ID and start building with Request Network"
---

## Welcome to Request Network

Get started with Request Network in just a few minutes. This guide will walk you through setting up your account, obtaining your Client ID, and making your first payment.
...
- Store your Client ID and webhook secret in environment variables
...
`RN_CLIENT_ID` is used server-side only — it's sent as the `x-client-id` header on API requests and should never be exposed in client-side code.
...
To track payment status in real-time, set up a webhook endpoint. Always verify against the **raw** request body, before it's parsed as JSON — re-serializing the parsed body will produce a different signature and fail verification.
...
const app = express();
// Capture the raw body alongside JSON parsing so we can verify against
// the exact bytes Request Network signed.
...
app.post('/webhooks', async (req, res) => {
  ...
  // Verify signature against the RAW body (not JSON.stringify(req.body))
